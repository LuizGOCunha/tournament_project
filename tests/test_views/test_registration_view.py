...
class TestRegistrationView:
    ...
    def test_to_see_if_we_can_create_user_with_correct_info(self, registration_data):
        request = self.factory.post(
            path=reverse("registration"), data=registration_data
        )
        ...
        ), "User's first name isn't correct"
        assert (
            user.last_name == registration_data["last_name"]
        ), "User's last name isn't correct"
        assert (
            user.username == registration_data["username"]
        ), "User's username isn't correct"
