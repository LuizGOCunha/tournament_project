...
class TestAddfighterView:
    ...
    def test_if_view_is_accessible(self, dummy_user, registration_data):
        ...
        assert response.status_code == 302, "Could not redirect when user is not authenticated"
        self.client.login(
    ...
    def test_if_we_can_create_fighter_for_user(self, dummy_user, fighter_django_data):
        request = self.factory.post(path=reverse("addfighter"),data=fighter_django_data,)
        ...
        assert fighter.name == fighter_django_data['name'], "Fighters name value is not correct"
        assert fighter.belt == fighter_django_data['belt'], "Fighters belt value is not correct"
        # Had to convert the value below to float, otherwise it would compare djangos Decimal class to float
        assert float(fighter.weight) == fighter_django_data['weight'], "Fighters weight value is not correct"
        assert fighter.age == fighter_django_data['age'], "Fighters age value is not correct"
        assert fighter.sex == fighter_django_data['sex'], "Fighters sex value is not correct"
