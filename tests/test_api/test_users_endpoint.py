...
class TestFighterEndpoint:
    ...
    def test_if_we_can_access_api(self):
        ...
        # Why??? Find out!
        # Partial answer: The first part is related to the basename assigned at router
        # I would assume the 'list' relates to the ViewSet method, but none of the others are accessible
        response = self.client.get(reverse("users-list"))
        assert response.status_code == 200

    def test_if_we_can_return_fighter_data_with_get(
        self, registration_data, dummy_user
    ):
        response = self.client.get(reverse("users-list"))
