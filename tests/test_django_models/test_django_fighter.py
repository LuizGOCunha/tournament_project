...
class TestFighterDjangoModel:
    def test_if_we_can_create_a_fighter_django_model(self, dummy_user):
        ...
        assert (
            FighterDjangoModel.objects.count() == 1
        ), "Fighter could not be created in database"
    ...
    def test_if_we_can_convert_djangomodel_to_tournamentmodel(
        ...
        assert (
            fightertournament.return_belt() == Belt(fighterdjango.belt).return_my_belt()
        )
